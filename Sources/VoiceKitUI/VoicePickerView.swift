...
public final class VoicePickerViewModel: ObservableObject {
    ...
    private var allowSystemVoices: Bool = false

    public init(tts: TTSConfigurable, store: VoiceProfilesStore) {
        self.tts = tts
        self.store = store
        // Snapshot test flag exactly once at init
        Task { @MainActor in
            self.allowSystemVoices = await VoiceKitTestMode.allowSystemVoiceQueries()
    ...
    public func refreshAvailableVoices() {
        voices = availableSystemVoices().sorted { a, b in
            if a.language == b.language { return a.name < b.name }
            return a.language < b.language
        }
    }

    private func availableSystemVoices() -> [TTSVoiceInfo] {
        if !allowSystemVoices {
            return [TTSVoiceInfo(id: "com.apple.speech.synthesis.voice.Alex", name: "Alex", language: "en-US")]
        }
        return AVSpeechSynthesisVoice.speechVoices().map { TTSVoiceInfo(id: $0.identifier, name: $0.name, language: $0.language) }
    }
    ...
    private func pickPreviewVoice() -> (String, TTSVoiceProfile) {
        ...
        if let info = (active.isEmpty ? filteredVoices : active).randomElement() { return (info.id, store.profile(for: info)) }
        let any = voices.first!; return (any.id, store.profile(for: any))
    }
