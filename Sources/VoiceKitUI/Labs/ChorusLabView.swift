...
struct ChorusLabView: View {
    ...
    /// Returns a complete snippet suitable for pasting in an app target.
    /// Includes a minimal import and a single array literal of TTSVoiceProfile one-liners.
    private func makeChorusSnippet(for profiles: [TTSVoiceProfile]) -> String {
        guard !profiles.isEmpty else {
            return "// No voices selected."
        }
        var lines: [String] = []
        lines.append("// VoiceKit chorus setup (generated by Chorus Lab)")
        lines.append("import VoiceKit")
        lines.append("")
        
        lines.append("let chorusProfiles = [")
        lines.append(profiles.map { "    " + $0.initStr() }
            .joined(separator: ",\n"))
        lines.append("]")
        
        // Optional: usage hint
        lines.append(#"// Use with VoiceChorus: await VoiceChorus().sing("Your phrase", withVoiceProfiles: chorusProfiles)"#)
