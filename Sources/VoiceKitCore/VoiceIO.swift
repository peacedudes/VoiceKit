...
//  - Audio tap avoids capturing @MainActor self directly; uses a @unchecked Sendable LevelSink wrapper to post level updates to main safely.
//  - Boosted playback uses off-main detached tasks with short, bounded waits.
//
...
private enum PermissionBridge {
    // Speech auth (TCC may call this on a background queue)
    nonisolated static func requestSpeechAuth(_ handler: @Sendable @escaping (SFSpeechRecognizerAuthorizationStatus) -> Void) {
        SFSpeechRecognizer.requestAuthorization(handler)
    }

    #if os(iOS)
    // iOS 17+: AVAudioApplication only (package min iOS is 17.0)
    nonisolated static func requestMicPermission_iOS17(_ handler: @Sendable @escaping (Bool) -> Void) {
        AVAudioApplication.requestRecordPermission(completionHandler: handler)
    }
    #elseif os(macOS)
    // macOS: AVCaptureDevice
    nonisolated static func requestMicPermission_macOS(_ handler: @Sendable @escaping (Bool) -> Void) {
...
public final class RealVoiceIO: NSObject, VoiceIO, TTSConfigurable {
    ...
    // Immutable delegate; avoids mutable Sendable warnings
    ...
    public func ensurePermissions() async throws {
        ...
        micOK = await withCheckedContinuation { (c: CheckedContinuation<Bool, Never>) in
            PermissionBridge.requestMicPermission_iOS17 { c.resume(returning: $0) }
        }
        #elseif os(macOS)
        micOK = await withCheckedContinuation { (c: CheckedContinuation<Bool, Never>) in
            PermissionBridge.requestMicPermission_macOS { c.resume(returning: $0) }
        }
        #else
        micOK = false
        #endif
        ...
        let speechAuth = await withCheckedContinuation { (c: CheckedContinuation<SFSpeechRecognizerAuthorizationStatus, Never>) in
            PermissionBridge.requestSpeechAuth { c.resume(returning: $0) }
        }
    ...
    public func listen(timeout: TimeInterval, inactivity: TimeInterval, record: Bool) async throws -> VoiceResult {
        ...
        // Thread-safe UI level sink (avoids capturing @MainActor self in the audio thread)
    ...
    public func startPreparedBoosted() async throws {
        ...
        try await withCheckedThrowingContinuation { (cont: CheckedContinuation<Void, Error>) in
            Task.detached(priority: .utility) { [weak self] in
                ...
                while true {
                    let done = await self.boostQueue.isDone()
                    if done { break }
                    if CACurrentMediaTime() - start >= timeout { break }
                    try? await Task.sleep(nanoseconds: 20_000_000)
                }
                await MainActor.run {
                    self.stopBoostedPlayback()
                    cont.resume()
                }
            }
        }
...
// MARK: - AVSpeechSynthesizer delegate (private, immutable owner)

private final class SpeechDelegate: NSObject, AVSpeechSynthesizerDelegate {
    // Immutable owner avoids “mutable stored property on Sendable class” warnings
    ...
    func speechSynthesizer(_ synthesizer: AVSpeechSynthesizer, willSpeakRangeOfSpeechString characterRange: NSRange, utterance: AVSpeechUtterance) {
        let text = utterance.speechString // copy sendable String
